#line 1 "..\\code\\user\\Font_lib_SD.c"
...
#line 2 "..\\code\\user\\Font_lib_SD.c"
...
#line 3 "..\\code\\user\\Font_lib_SD.c"
...
#line 4 "..\\code\\user\\Font_lib_SD.c"
...
void pg_up_dn(void);
#line 5 "..\\code\\user\\Font_lib_SD.c"
u8 GBK16[32];
u16 LCD_X=0,LCD_Y=0;
char end_ebook;
unsigned char *Read_One_GBK16(unsigned char *ch) 
{ 
   ...
   if(*ch<0x81)
   {
	...
	{ 	 
	temp1=(94*(*ch-1-0xa0)+(*(ch+1)-1-0xa0+112))*32;
	f_lseek(&fsrc,temp1);
...
void Lcd_WriteASCIIClarity(u16 x,u8 y,u16 CharColor,u16 CharBackColor,u8 *ASCIICode,u8 flag)
{
  ...
  ASCIIPointer=Read_One_GBK16(ASCIICode);
	   ...
	   if(*s<0x81) 
		   {
			...
			if(*s==0x0D)	
			   {
			   	if(*(s+1)=='\0') {LCD_X=0;LCD_Y=0;return 1;} 
		...
		if (x<39*8) x=x+8;  
	       ...
	       {
	         x=0;y=0;
	         pg_up_dn();if(end_ebook)return;Lcd_Clear(0x0000);		
	       } 
...
void Ebook(char *filename)
{
	...
	LCD_X=0;LCD_Y=0;
	f_mount(0, &fs);
  	f_open(&fsrc,filename, 0x00 | 0x01);
	Lcd_Clear(0x0000);
	f_lseek(&fsrc,0);
	ty=fsrc.fsize;
	end_ebook=0;
